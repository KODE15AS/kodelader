...
- **Fase 2 (benketest fullført 2026-08, sandkassen ferdig verifisert):** Hele kjeden testet ende-til-ende på ekte maskinvare (Shelly + kontaktor + 230 V-last): QR → Vipps → lading → auto-/manuell avslutning → delvis capture → SMS med kvittering. Inkludert kun-Vipps-checkout, cookie-parring for gjengangere og forhåndsutfylt Vipps-nummer. Autonomi-scriptet (lokal utkobling ved grense, også uten nett) er bygget og verifisert med simulator — installeres på enheten fra admin over WebSocket. Gjenstår: autonomitest på ekte maskinvare, overgang til skarp Vipps. Trefaseinstallasjon 400 V med elektriker er utsatt
